      ...
      <div className='flex flex-col sm:flex-row gap-8 mb-8'>
        ...
        <div className='flex-shrink-0 w-full sm:w-32 flex flex-col items-center justify-center bg-muted/30 rounded-xl p-4 border border-border'>
          <div className='relative w-24 h-24'>
            <svg className={cn('circular-chart', matchColor)} viewBox='0 0 36 36'>
              <path
                className='circle-bg'
            ...
            <ul className='space-y-2.5'>
              {strengths.map((str, i) => (
                <li key={i} className='flex items-center gap-2 text-sm font-medium text-muted-foreground'>
                  <div className='w-1.5 h-1.5 rounded-full bg-emerald-500' /> {str}
                </li>
              ))}
            </ul>
          </div>
          <div>
            <p className='text-[10px] font-bold text-orange-600 uppercase tracking-widest mb-3 flex items-center gap-1'>
            ...
            <ul className='space-y-2.5'>
              {missing.map((mis, i) => (
                <li key={i} className='flex items-center gap-2 text-sm font-medium text-muted-foreground opacity-70'>
                  <div className='w-1.5 h-1.5 rounded-full bg-orange-500' /> {mis}
                </li>
              ))}
            </ul>
          </div>
