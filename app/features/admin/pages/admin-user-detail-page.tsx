      ...
      <section className='bg-card rounded-2xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border border-border shadow-sm relative overflow-hidden group'>
        ...
        <div className='flex flex-col sm:flex-row items-center gap-6 relative z-10'>
          <div className='h-24 w-24 rounded-2xl overflow-hidden shadow-sm border border-border shrink-0'>
            <img alt={detail.name} className='h-full w-full object-cover' src={detail.avatarUrl} />
          </div>
        ...
        <div className='flex gap-3 w-full md:w-auto relative z-10 shrink-0'>
          <button
            ...
            className={`flex-1 md:flex-initial px-6 py-2.5 rounded-full font-bold text-sm transition-all border cursor-pointer border-destructive hover:bg-destructive/10 ${
              banned ? 'opacity-50 pointer-events-none' : 'text-destructive'
            }`}
            disabled={banned}
          >
          ...
          <button
            ...
            className={`flex-1 md:flex-initial px-6 py-2.5 rounded-full font-bold text-sm transition-all border cursor-pointer ${
              banned
        ...
        <section className='lg:col-span-5 bg-card rounded-2xl p-8 flex flex-col border border-border shadow-sm relative overflow-hidden group'>
          ...
          <div className='space-y-8 flex-1 relative z-10'>
            ...
            <div className='group'>
              <div className='flex justify-between items-end mb-3'>
                <div>
                  <p className='text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1'>Roadmaps</p>
                  <p className='text-lg font-bold text-foreground'>
              ...
              <div className='h-2.5 w-full bg-muted rounded-full overflow-hidden border border-border'>
                <div
                  ...
                  style={{ width: `${detail.usage.roadmaps.percent}%` }}
                />
              </div>
            </div>
            ...
            <div className='group'>
              <div className='flex justify-between items-end mb-3'>
                <div>
                  <p className='text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1'>Assessments</p>
                  <p className='text-lg font-bold text-foreground'>
                    {detail.usage.assessments.current} /{' '}
                    <span className='text-primary'>{detail.usage.assessments.max}</span>
                  </p>
                </div>
                <span className='material-symbols-outlined text-primary group-hover:scale-110 transition-transform'>
                  assessment
              ...
              <div className='h-2.5 w-full bg-muted rounded-full overflow-hidden border border-border'>
                <div
                  className='h-full bg-primary rounded-full'
                  style={{ width: `${detail.usage.assessments.percent}%` }}
                />
              </div>
            </div>
          </div>
      ...
      <section className='bg-card rounded-2xl p-8 border border-border shadow-sm overflow-hidden relative group'>
        ...
        <div className='overflow-x-auto'>
          {detail.payments.length === 0 ? (
            <div className='py-8 text-center text-muted-foreground font-medium'>No transactions recorded</div>
          ) : (
