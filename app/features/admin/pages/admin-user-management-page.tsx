...
export function AdminUserManagementPage() {
  ...
  const totalPages = useMemo(() => Math.max(1, Math.ceil((total || rows.length || 1) / pageSize)), [pageSize, rows.length, total])
      ...
      <section className='bg-card rounded-xl p-6 shadow-sm'>
        <div className='grid grid-cols-1 md:grid-cols-4 gap-6'>
          <div className='space-y-2 md:col-span-2'>
            <label className='text-xs font-bold uppercase tracking-widest text-muted-foreground'>{t('users.filter.searchLabel')}</label>
            <input className='w-full px-4 py-3 bg-muted border-none rounded-lg focus:ring-2 focus:ring-primary/50 focus-visible:outline-none' placeholder={t('users.filter.searchPlaceholder')} type='text' value={query} onChange={(e) => { setPage(1); setQuery(e.target.value) }} />
          </div>
          <div className='space-y-2'>
            <label className='text-xs font-bold uppercase tracking-widest text-muted-foreground'>{t('users.filter.tierLabel')}</label>
            <select value={planFilter} onChange={(e) => { setPage(1); setPlanFilter(e.target.value) }} className='w-full px-4 py-3 bg-muted border-none rounded-lg focus:ring-2 focus:ring-primary/50 focus-visible:outline-none appearance-none'>
          ...
          <div className='space-y-2'>
            <label className='text-xs font-bold uppercase tracking-widest text-muted-foreground'>{t('users.filter.statusLabel')}</label>
            <select value={statusFilter} onChange={(e) => { setPage(1); setStatusFilter(e.target.value) }} className='w-full px-4 py-3 bg-muted border-none rounded-lg focus:ring-2 focus:ring-primary/50 focus-visible:outline-none appearance-none'>
          ...
          <table className='w-full text-left border-collapse'>
            <thead>
              <tr className='bg-muted/50'>
                <th className='px-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground border-none'>{t('users.table.user')}</th>
                <th className='px-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground border-none'>{t('users.table.tier')}</th>
                <th className='px-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground border-none'>{t('users.table.registered')}</th>
                <th className='px-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground border-none'>{t('users.table.jdCount')}</th>
                <th className='px-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground border-none'>{t('users.table.status')}</th>
                <th className='px-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground border-none text-right'>{t('users.table.actions')}</th>
              </tr>
            </thead>
              ...
              {loading ? (
                Array.from({ length: 5 }).map((_, index) => (
                  <tr key={index} className='animate-pulse'>
                    ...
                    <td className='px-6 py-5'><div className='h-4 w-24 rounded bg-muted' /></td>
                    <td className='px-6 py-5'><div className='h-4 w-8 rounded bg-muted' /></td>
                    <td className='px-6 py-5'><div className='h-6 w-20 rounded-lg bg-muted' /></td>
                    <td className='px-6 py-5'>
