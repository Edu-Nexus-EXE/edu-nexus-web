...
export function AdminSkillsQueuePage() {
  ...
  function handleSubmitSkill() {
    ...
    openConfirm({
      title: editing
        ? label('Lưu thay đổi kỹ năng?', 'Save skill changes?')
        : label('Tạo kỹ năng mới?', 'Create new skill?'),
      message: editing
        ? label(
            `Metadata và trạng thái của "${form.name.trim()}" sẽ được cập nhật trong taxonomy.`,
            `Metadata and status for "${form.name.trim()}" will be updated in the taxonomy.`
          )
        : label(
            `"${form.name.trim()}" sẽ được thêm vào taxonomy kỹ năng của hệ thống.`,
            `"${form.name.trim()}" will be added to the system skill taxonomy.`
          ),
  ...
  function handleApprove(row: AdminSkillRowView) {
    openConfirm({
      ...
      message: label(
        `"${row.title}" sẽ rời hàng đợi duyệt và được phép dùng trong hệ thống.`,
        `"${row.title}" will leave the review queue and become available in the system.`
      ),
  ...
  function handleToggleActive(row: AdminSkillRowView) {
    ...
    openConfirm({
      ...
      message: nextActive
        ? label(
            `"${row.title}" sẽ được dùng lại trong taxonomy, roadmap và gợi ý liên quan.`,
            `"${row.title}" will be available again for taxonomy, roadmaps, and recommendations.`
          )
        : label(
            `"${row.title}" sẽ bị ẩn khỏi hệ thống sử dụng mới, nhưng dữ liệu lịch sử vẫn được giữ.`,
            `"${row.title}" will be hidden from new system use, while history remains intact.`
          ),
      ...
      onConfirm: async () => {
        try {
          ...
          setError(
            (err as Error).message || label('Không thể cập nhật trạng thái kỹ năng.', 'Failed to update skill status.')
          )
          throw err
        } finally {
          setBusyId(null)
  ...
  function handleAddPrereq() {
    ...
    openConfirm({
      ...
      message: label(
        `"${target?.title ?? prereqSkillId}" sẽ trở thành kỹ năng cần học trước "${editing.title}".`,
        `"${target?.title ?? prereqSkillId}" will become a prerequisite for "${editing.title}".`
      ),
  ...
  function handleRemovePrereq(prereqId: string) {
    ...
    openConfirm({
      ...
      message: label(
        `"${target?.name ?? prereqId}" sẽ không còn là điều kiện học trước của "${editing.title}".`,
        `"${target?.name ?? prereqId}" will no longer be a prerequisite for "${editing.title}".`
      ),
      ...
      onConfirm: async () => {
        try {
          ...
          setError(
            (err as Error).message || label('Không thể xóa kỹ năng tiên quyết.', 'Failed to remove prerequisite.')
          )
          throw err
        } finally {
          setBusyId(null)
  ...
  function handleMerge(row: AdminSkillRowView) {
    if (!mergeTargetId.trim() || mergeTargetId.trim() === row.id) {
      setError(
        label(
          'Chọn kỹ năng đích khác kỹ năng hiện tại để gộp.',
          'Choose a target skill different from the current skill.'
        )
      )
      return
    }
    ...
    openConfirm({
      ...
      message: label(
        `"${row.title}" sẽ được gộp vào "${target?.title ?? mergeTargetId}". Toàn bộ usage sẽ chuyển sang kỹ năng đích và kỹ năng cũ sẽ bị xóa.`,
        `"${row.title}" will be merged into "${target?.title ?? mergeTargetId}". All usage moves to the target skill and the old skill will be deleted.`
      ),
    ...
    return (
      <div className='flex justify-end gap-2'>
        <button
          ...
          title={t('skillsQueue.viewDetails')}
          aria-label={t('skillsQueue.viewDetails')}
        >
        ...
        <button
          ...
          title={t('skillsQueue.edit')}
          aria-label={t('skillsQueue.edit')}
        >
        ...
        {variant === 'queue' ? (
          <button
            ...
            title={t('skillsQueue.approve')}
            aria-label={t('skillsQueue.approve')}
          >
        ...
        >
          <span className='material-symbols-outlined text-[18px]'>
            {row.isActive ? 'block' : 'settings_backup_restore'}
          </span>
        </button>
      </div>
    )
      ...
      <header className='flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between'>
        <div>
          <h1 className='text-4xl font-bold text-foreground'>
            {label('Phân loại kỹ năng & hàng đợi duyệt', 'Skills Taxonomy & Review Queue')}
          </h1>
          <p className='mt-2 text-muted-foreground'>
            {label(
              'Quản lý bộ kỹ năng, quan hệ kỹ năng tiên quyết và hàng đợi kỹ năng do AI tạo.',
              'Manage skills taxonomy, prerequisite graph, and AI-generated skill review queue.'
            )}
          </p>
        </div>
        <button
      ...
      <section className='overflow-hidden rounded-2xl border border-warning/20 bg-card shadow-sm'>
        <div className='flex flex-col gap-3 border-b border-border bg-warning/5 px-5 py-4 md:flex-row md:items-center md:justify-between'>
          <div>
            <h2 className='text-lg font-bold text-foreground'>
              {label('Hàng đợi duyệt kỹ năng', 'Skill Review Queue')}
            </h2>
            <p className='mt-1 text-sm text-muted-foreground'>
              {label(
                'Ưu tiên xử lý các kỹ năng AI tạo trước khi chúng được dùng trong hệ thống.',
                'Prioritize AI-generated skills before they become available in the system.'
              )}
            </p>
          </div>
        ...
        <div className='divide-y divide-border'>
          {queueLoading ? (
            ...
            <div className='p-8 text-center text-sm text-muted-foreground'>
              {label('Không có kỹ năng chờ duyệt.', 'No skills pending review.')}
            </div>
          ) : (
            queue.map((row) => (
                ...
                <div className='flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between'>
                  <div className='min-w-0'>
                    ...
                    <p className='mt-1 text-xs text-muted-foreground'>
                      {localizeSkillValue(row.subtitle, language, SKILL_CATEGORY_VI)} · {row.major || 'IT'} ·{' '}
                      {label('Lộ trình', 'Roadmap')} {row.roadmapUsage ?? 0} · {label('Tài nguyên', 'Resource')}{' '}
                      {row.resourceCount ?? 0}
                    </p>
                    <p className='mt-2 line-clamp-2 text-sm text-muted-foreground'>
                      {row.description || label('Chưa có mô tả.', 'No description yet.')}
                    </p>
                  </div>
        ...
        <div className='flex items-center justify-between border-t border-border bg-muted/30 px-5 py-4'>
          <p className='text-xs font-semibold text-muted-foreground'>
            {t('adminCommon.pagination', { page: queuePage, totalPages: queuePages, total: queueTotal })}
          </p>
          <div className='flex gap-2'>
            ...
            >
              {label('Trước', 'Previous')}
            </button>
            <button
              type='button'
              disabled={queuePage >= queuePages}
              onClick={() => setQueuePage((current) => Math.min(queuePages, current + 1))}
              className='rounded-lg border border-border px-3 py-2 text-sm disabled:opacity-50'
            >
              {label('Sau', 'Next')}
            </button>
      ...
      <section className='rounded-2xl border border-border bg-card p-4 shadow-sm'>
        <div className='grid grid-cols-1 gap-3 md:grid-cols-4'>
          <input
            ...
            placeholder={label('Tìm kỹ năng...', 'Search skills...')}
          ...
          <input
            ...
            placeholder={label('Danh mục, ví dụ framework', 'Category, e.g. framework')}
            className='rounded-xl border border-border bg-muted px-4 py-3 text-sm outline-none focus:border-primary'
          />
          <input
            value={major}
            ...
            placeholder={label('Ngành, ví dụ IT', 'Major, e.g. IT')}
            className='rounded-xl border border-border bg-muted px-4 py-3 text-sm outline-none focus:border-primary'
          />
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={5} className='px-5 py-10 text-center text-sm text-muted-foreground'>
                        {label('Không có kỹ năng.', 'No skills found.')}
                      </td>
                    </tr>
                  ) : (
                    skills.map((row) => (
                      <tr key={row.id} className='align-top hover:bg-muted/30'>
                        ...
                        <td className='px-5 py-4 text-sm text-muted-foreground'>
                          ...
                          <p className='text-xs'>
                            {row.major || 'IT'} · {label('Độ khó', 'Level')} {row.difficultyLevel ?? 3}
                          </p>
                        </td>
                        <td className='px-5 py-4 text-xs text-muted-foreground'>
                          <p>
                            {label('Lộ trình', 'Roadmap')}: {row.roadmapUsage ?? 0}
                          </p>
                          <p>JD: {row.jdUsage ?? 0}</p>
                          <p>
                            {label('Tài nguyên', 'Resources')}: {row.resourceCount ?? 0}
                          </p>
                        </td>
                        <td className='px-5 py-4'>
                        ...
                        <td className='px-5 py-4'>{renderSkillActions(row, 'list')}</td>
                      </tr>
                    ))
                  )}
                </tbody>
            ...
            <aside className='max-h-[90vh] w-full max-w-3xl space-y-4 overflow-y-auto rounded-3xl border border-border bg-background p-6 shadow-2xl'>
              <section className='rounded-2xl border border-border bg-card p-5 shadow-sm'>
                <div className='flex items-start justify-between gap-4'>
                  <div>
                    <h2 className='text-lg font-bold text-foreground'>
                      {isViewing
                        ? t('skillsQueue.form.viewTitle')
                        : editing
                          ? t('skillsQueue.form.editTitle')
                          : t('skillsQueue.form.createTitle')}
                    </h2>
                    <p className='mt-1 text-sm text-muted-foreground'>
                      {isViewing ? t('skillsQueue.form.viewDesc') : t('skillsQueue.form.editDesc')}
                    </p>
                  </div>
                  <button
                    type='button'
                    ...
                    aria-label={t('skillsQueue.cancel')}
                  >
                ...
                >
                  <div className='space-y-1.5'>
                    ...
                    <input
                      ...
                      placeholder={label('Tên kỹ năng', 'Skill name')}
                  ...
                  <div className='space-y-1.5'>
                    <label className='text-xs font-bold uppercase tracking-wider text-muted-foreground'>
                      {t('skillsQueue.form.slug')}
                    </label>
                    <input
                      value={form.slug}
                      onChange={(event) => setForm((current) => ({ ...current, slug: event.target.value }))}
                      placeholder={label('duong-dan-ky-nang', 'skill-slug')}
                      className='w-full rounded-xl border border-border bg-background px-4 py-3 text-sm disabled:opacity-60'
                      disabled={isViewing}
                    />
                  </div>
                  <div className='grid grid-cols-2 gap-3'>
                    <div className='space-y-1.5'>
                      <label className='text-xs font-bold uppercase tracking-wider text-muted-foreground'>
                        {t('skillsQueue.form.category')}
                      </label>
                      <input
                        value={form.category}
                        onChange={(event) => setForm((current) => ({ ...current, category: event.target.value }))}
                        placeholder={label('Danh mục kỹ năng', 'Skill category')}
                        className='w-full rounded-xl border border-border bg-background px-4 py-3 text-sm disabled:opacity-60'
                        disabled={isViewing}
                      />
                    </div>
                    <div className='space-y-1.5'>
                      <label className='text-xs font-bold uppercase tracking-wider text-muted-foreground'>
                        {t('skillsQueue.form.major')}
                      </label>
                      <input
                        value={form.major}
                        onChange={(event) => setForm((current) => ({ ...current, major: event.target.value }))}
                        placeholder={label('Ngành học, ví dụ IT', 'Major, e.g. IT')}
                        className='w-full rounded-xl border border-border bg-background px-4 py-3 text-sm disabled:opacity-60'
                        disabled={isViewing}
                      />
                    </div>
                  ...
                  <div className='space-y-1.5'>
                    <label className='text-xs font-bold uppercase tracking-wider text-muted-foreground'>
                      {t('skillsQueue.form.description')}
                    </label>
                    <textarea
                      value={form.description}
                      onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))}
                      placeholder={label('Mô tả', 'Description')}
                      className='min-h-24 w-full rounded-xl border border-border bg-background px-4 py-3 text-sm disabled:opacity-60'
                      disabled={isViewing}
                    />
                  </div>
                  <div className='space-y-1.5'>
                    <label className='text-xs font-bold uppercase tracking-wider text-muted-foreground'>
                      {t('skillsQueue.form.difficultyLevel')}
                    </label>
                    <input
                      ...
                      min='1'
                      max='5'
                      placeholder={label('Độ khó 1-5', 'Difficulty 1-5')}
                      className='w-full rounded-xl border border-border bg-background px-4 py-3 text-sm disabled:opacity-60'
                      disabled={isViewing}
                    />
                  </div>
              ...
              {editing && !isViewing ? (
                <>
                  <section className='rounded-2xl border border-border bg-card p-5 shadow-sm'>
                    <h2 className='text-lg font-bold text-foreground'>
                      {label('Kỹ năng tiên quyết', 'Prerequisites')}
                    </h2>
                    <p className='mt-1 text-sm text-muted-foreground'>
                      {label(
                        'Hệ thống sẽ kiểm tra vòng lặp trước khi lưu quan hệ tiên quyết.',
                        'Backend checks cycles before saving.'
                      )}
                    </p>
                    <div className='mt-4 space-y-3'>
                      <input
                        value={skillPickerSearch}
                        onChange={(event) => setSkillPickerSearch(event.target.value)}
                        placeholder={label('Tìm kỹ năng...', 'Search skills...')}
                      ...
                      <div className='flex gap-2'>
                        <select
                          value={prereqSkillId}
                          onChange={(event) => setPrereqSkillId(event.target.value)}
                      ...
                      {skillPickerLoading ? (
                        <p className='text-xs text-muted-foreground'>{t('adminCommon.loading')}</p>
                      ) : null}
                    </div>
                    <div className='mt-4 space-y-2'>
                      {(editing.prerequisites ?? []).length === 0 ? (
                        <p className='text-sm text-muted-foreground'>
                          {label(
                            'Chưa có kỹ năng tiên quyết trong response hiện tại.',
                            'No prerequisites in the current response.'
                          )}
                        </p>
                      ) : (
                        (editing.prerequisites ?? []).map((item) => (
                          ...
                          >
                            ...
                            <button
                              type='button'
                              onClick={() => void handleRemovePrereq(item.id)}
                  ...
                  <section className='rounded-2xl border border-border bg-card p-5 shadow-sm'>
                    <h2 className='text-lg font-bold text-foreground'>
                      {label('Gộp kỹ năng trùng', 'Merge duplicate')}
                    </h2>
                    <p className='mt-1 text-sm text-muted-foreground'>
                      {label(
                        'Chuyển toàn bộ nơi đang dùng sang kỹ năng đích rồi xóa kỹ năng cũ.',
                        'Move all usage references to the target skill, then delete the old skill.'
                      )}
                    </p>
                    <div className='mt-4 space-y-3'>
                      <select
                      ...
                      <input
                        ...
                        placeholder={label('Lý do gộp', 'Merge reason')}
                        className='w-full rounded-xl border border-border bg-background px-4 py-3 text-sm'
                      />
                      <button
                        type='button'
                        disabled={busyId === editing.id}
                        onClick={() => void handleMerge(editing)}
                        className='w-full rounded-xl border border-warning/40 px-4 py-3 text-sm font-bold text-warning disabled:opacity-50'
                      >
