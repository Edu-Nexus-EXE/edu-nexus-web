import type { AdminMarketJobDetailView } from '../../lib/market-readiness'
...
type Props = {
  ...
  loading: boolean
  error: string
  job: AdminMarketJobDetailView | null
  copyState: CopyState
  labels: Labels
  onClose: () => void
...
function getDisplayContent(job: AdminMarketJobDetailView) {
...
export function AdminMarketJobDetailDrawer({ open, loading, error, job, copyState, labels, onClose, onCopy }: Props) {
  if (!open) return null
          ...
          <div className='flex items-start justify-between gap-4'>
            <div className='min-w-0'>
              ...
              {loading ? (
                ...
                <h2 className='mt-1 text-2xl font-black leading-tight text-foreground'>{job?.jobTitle ?? '-'}</h2>
              )}
              {job ? (
          ...
          {error ? (
            ...
            <div className='space-y-6'>
              ...
              <details className='rounded-2xl border border-border bg-muted/20 p-4'>
                <summary className='cursor-pointer text-sm font-black text-foreground'>{labels.technical}</summary>
                <div className='mt-4 grid gap-3 sm:grid-cols-3'>
                  <div className='rounded-xl border border-border bg-background p-4'>
                    <p className='text-xs font-bold uppercase tracking-widest text-muted-foreground'>
                      {labels.quality}
                    </p>
                    <p className='mt-1 text-sm font-black text-foreground'>{job.originalContentQuality ?? '-'}</p>
                  </div>
                  <div className='rounded-xl border border-border bg-background p-4'>
                    <p className='text-xs font-bold uppercase tracking-widest text-muted-foreground'>{labels.length}</p>
                    <p className='mt-1 text-sm font-black text-foreground'>{job.contentLength ?? '-'}</p>
                  </div>
                  <div className='rounded-xl border border-border bg-background p-4'>
                    <p className='text-xs font-bold uppercase tracking-widest text-muted-foreground'>{labels.hash}</p>
                    <p className='mt-1 break-all font-mono text-xs text-foreground'>{job.rawContentHash ?? '-'}</p>
                  </div>
                </div>
              </details>
            </div>
